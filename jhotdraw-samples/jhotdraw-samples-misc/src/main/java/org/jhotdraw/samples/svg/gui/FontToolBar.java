...
public class FontToolBar extends AbstractToolBar {
    ...
    protected JComponent createDisclosedComponent(int state) {
        // Compose Method: each disclosure state is now just "which layout
        // constants to use", not a hand-rolled copy of the whole panel.
        switch (state) {
            case 1:
                return buildFontPanel(2, 2);
            case 2:
                return buildFontPanel(12, 3);
            default:
                return null;
        }
    }
    ...
    /**
     * Builds the font panel shown for a disclosure state. {@code faceColumns}
     * and {@code gridwidth} are the only two values that ever differed
     * between the former per-state copies of this panel (Parameterize
     * Method).
     */
    private JPanel buildFontPanel(int faceColumns, int gridwidth) {
        JPanel p = new JPanel();
        p.setOpaque(false);
        ...
        ResourceBundleUtil labels = ResourceBundleUtil.getBundle("org.jhotdraw.samples.svg.Labels");
        addFaceField(p, labels, faceColumns, gridwidth);
        addSizeControls(p, labels);
        addFontStyleButtons(p, labels);
        return p;
    }

    /**
     * Adds the font face field and its popup chooser button to {@code p}.
     */
    private void addFaceField(JPanel p, ResourceBundleUtil labels, int faceColumns, int gridwidth) {
        JAttributeTextField<Font> faceField = new JAttributeTextField<Font>();
        faceField.setColumns(faceColumns);
    ...
    /**
     * Adds the font size field and its popup slider to {@code p}.
     */
    private void addSizeControls(JPanel p, ResourceBundleUtil labels) {
    ...
    /**
     * Adds the bold/italic/underline style buttons to {@code p}.
     */
